...
func (suite *AnteTestSuite) TestAnteHandlerFees() {
	...
	testCases := []struct {
		...
		},
		{
			...
			func() {
				err := simapp.FundAccount(suite.app.BankKeeper, suite.ctx, addr0, sdk.NewCoins(sdk.NewInt64Coin("unos", 149)))
				suite.Require().NoError(err)
			},
			false,
			false,
		...
		},
		{
			...
			func() {
				...
				require.True(sdk.IntEq(suite.T(), suite.app.BankKeeper.GetAllBalances(suite.ctx, addr0).AmountOf("unos"), sdk.NewInt(149)))

				err := simapp.FundAccount(suite.app.BankKeeper, suite.ctx, addr0, sdk.NewCoins(sdk.NewInt64Coin("unos", 1)))
				suite.Require().NoError(err)
			},
			false,
			true,
		...
		},
		{
			...
			func() {
				...
				require.True(sdk.IntEq(suite.T(), suite.app.BankKeeper.GetAllBalances(suite.ctx, modAcc.GetAddress()).AmountOf("unos"), sdk.NewInt(150)))
				require.True(sdk.IntEq(suite.T(), suite.app.BankKeeper.GetAllBalances(suite.ctx, addr0).AmountOf("unos"), sdk.NewInt(0)))
			},
			false,
			false,
			sdkerrors.ErrInsufficientFunds,
	...
	testCases := []TestCase{
		{
			...
			func() {
				feeAmount = sdk.NewCoins(sdk.NewInt64Coin("unos", 0))
				gasLimit = 0
			},
			false,
			false,
...
func (suite *AnteTestSuite) TestAnteHandlerReCheck() {
	...
	feeParam.GlobalMinimumGasPrices = sdk.NewDecCoins(sdk.NewDecCoinFromDec("unos", sdk.NewDecWithPrec(1, 5)))
