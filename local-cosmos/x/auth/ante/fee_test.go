...
func (suite *AnteTestSuite) TestEnsureMempoolFees() {
	...
	feeParam.GlobalMinimumGasPrices = sdk.NewDecCoins(
		...
		sdk.NewDecCoinFromDec("unos", sdk.NewDec(0)),
	)
	...
	coins := sdk.NewCoins(sdk.NewCoin("unos", sdk.NewInt(300)))
	...
	atomPrice := sdk.NewDecCoinFromDec("unos", sdk.NewDec(20))
	...
	atomPrice = sdk.NewDecCoinFromDec("unos", sdk.NewDec(0).Quo(sdk.NewDec(100000)))
...
func (suite *AnteTestSuite) TestDeductFees() {
	...
	coins := sdk.NewCoins(sdk.NewCoin("unos", sdk.NewInt(10)))
	err = simapp.FundAccount(suite.app.BankKeeper, suite.ctx, addr1, coins)
	suite.Require().NoError(err)
	...
	suite.app.AccountKeeper.SetAccount(suite.ctx, acc)
	err = simapp.FundAccount(suite.app.BankKeeper, suite.ctx, addr1, sdk.NewCoins(sdk.NewCoin("unos", sdk.NewInt(200))))
	suite.Require().NoError(err)
...
func (suite *AnteTestSuite) TestLazySendToModuleAccount() {
	...
	acc := suite.app.AccountKeeper.NewAccountWithAddress(suite.ctx, addr1)
	suite.app.AccountKeeper.SetAccount(suite.ctx, acc)
	err = simapp.FundAccount(suite.app.BankKeeper, suite.ctx, addr1, sdk.NewCoins(sdk.NewCoin("unos", sdk.NewInt(900))))
	suite.Require().NoError(err)

	feeCollectorAcc := suite.app.AccountKeeper.GetModuleAccount(suite.ctx, types.FeeCollectorName)
	expectedFeeCollectorBalance := suite.app.BankKeeper.GetBalance(suite.ctx, feeCollectorAcc.GetAddress(), "unos")
	...
	resultFeeCollectorBalance := suite.app.BankKeeper.GetBalance(suite.ctx, feeCollectorAcc.GetAddress(), "unos")
	suite.Assert().Equal(
	...
	depositFeeCollectorBalance := suite.app.BankKeeper.GetBalance(suite.ctx, feeCollectorAcc.GetAddress(), "unos")

	expectedAtomFee := feeAmount.AmountOf("unos")

	suite.Assert().Equal(
		// Called antehandler twice, expect fees to be deducted twice
		expectedFeeCollectorBalance.Add(sdk.NewCoin("unos", expectedAtomFee)).Add(sdk.NewCoin("unos", expectedAtomFee)),
...
func (suite *AnteTestSuite) TestGlobalMinimumFees() {
	...
	// keys and addresses
	priv1, _, addr1 := testdata.KeyTestPubAddr()
	coins := sdk.NewCoins(sdk.NewCoin("unos", sdk.NewInt(3000000000)))
	err := simapp.FundAccount(suite.app.BankKeeper, suite.ctx, addr1, coins)
	suite.Require().NoError(err)

	// msg and signatures
	msg := testdata.NewTestMsg(addr1)
	tx, err := suite.createTestTxWithGas(msg, 1500, 15000, priv1, "unos")
	suite.Require().NoError(err)

	// Global minimum gas price is zero, but transaction fee is non-zero
	feeParam := suite.app.ParamsKeeper.GetFeesParams(suite.ctx)
	feeParam.GlobalMinimumGasPrices = sdk.NewDecCoins(sdk.NewDecCoinFromDec("unos", sdk.NewDec(0)))
	suite.app.ParamsKeeper.SetFeesParams(suite.ctx, feeParam)
	suite.ctx = suite.ctx.WithMinGasPrices([]sdk.DecCoin{sdk.NewDecCoinFromDec("unos", sdk.NewDec(1000000000))})
	_, err = antehandler(suite.ctx, tx, false)
	suite.Assert().ErrorContains(err, "insufficient fees")

	// Global minimum gas price is non-zero, but transaction fee is zero
	feeParam = suite.app.ParamsKeeper.GetFeesParams(suite.ctx)
	feeParam.GlobalMinimumGasPrices = sdk.NewDecCoins(sdk.NewDecCoinFromDec("unos", sdk.NewDec(1000000000)))
	suite.app.ParamsKeeper.SetFeesParams(suite.ctx, feeParam)
	suite.ctx = suite.ctx.WithMinGasPrices([]sdk.DecCoin{sdk.NewDecCoinFromDec("unos", sdk.NewDec(0))})
	_, err = antehandler(suite.ctx, tx, false)
	suite.Assert().ErrorContains(err, "insufficient fees")

	// Global minimum gas price is non-zero, and transaction fee is non-zero but less than global minimum gas price
	feeParam = suite.app.ParamsKeeper.GetFeesParams(suite.ctx)
	feeParam.GlobalMinimumGasPrices = sdk.NewDecCoins(sdk.NewDecCoinFromDec("unos", sdk.NewDec(100)))
	suite.app.ParamsKeeper.SetFeesParams(suite.ctx, feeParam)
	suite.ctx = suite.ctx.WithMinGasPrices([]sdk.DecCoin{sdk.NewDecCoinFromDec("unos", sdk.NewDec(1))})
	_, err = antehandler(suite.ctx, tx, false)
	suite.Assert().ErrorContains(err, "insufficient fees")

	// Global minimum gas price is non-zero, and transaction fee is non-zero but less than global minimum gas price
	feeParam = suite.app.ParamsKeeper.GetFeesParams(suite.ctx)
	feeParam.GlobalMinimumGasPrices = sdk.NewDecCoins(sdk.NewDecCoinFromDec("unos", sdk.NewDec(1)))
	suite.app.ParamsKeeper.SetFeesParams(suite.ctx, feeParam)
	suite.ctx = suite.ctx.WithMinGasPrices([]sdk.DecCoin{sdk.NewDecCoinFromDec("unos", sdk.NewDec(100))})
	_, err = antehandler(suite.ctx, tx, false)
	suite.Assert().ErrorContains(err, "insufficient fees")

	// Global minimum gas price is non-zero, and transaction fee is equal to global minimum gas price
	feeParam = suite.app.ParamsKeeper.GetFeesParams(suite.ctx)
	feeParam.GlobalMinimumGasPrices = sdk.NewDecCoins(sdk.NewDecCoinFromDec("unos", sdk.NewDec(50)))
	suite.app.ParamsKeeper.SetFeesParams(suite.ctx, feeParam)
	suite.ctx = suite.ctx.WithMinGasPrices([]sdk.DecCoin{sdk.NewDecCoinFromDec("unos", sdk.NewDec(50))})
	// 750000 = 15000 * 50
	tx, _ = suite.createTestTxWithGas(msg, 750000, 15000, priv1, "unos")
	_, err = antehandler(suite.ctx, tx, false)
...
func (suite *AnteTestSuite) TestDeductFeeDependency() {
	...
	err = simapp.FundAccount(suite.app.BankKeeper, suite.ctx, addr2, sdk.NewCoins(sdk.NewCoin("unos", sdk.NewInt(200))))
	suite.Require().NoError(err)
	// create fee grant
...
func (suite *AnteTestSuite) TestMultipleGlobalMinimumFees() {
	...
	// keys and addresses
	priv1, _, addr1 := testdata.KeyTestPubAddr()
	coins := sdk.NewCoins(sdk.NewCoin("atom", sdk.NewInt(3000000000)), sdk.NewCoin("unos", sdk.NewInt(3000000000)))
	err := simapp.FundAccount(suite.app.BankKeeper, suite.ctx, addr1, coins)
	suite.Require().NoError(err)
	...
	suite.app.ParamsKeeper.SetFeesParams(suite.ctx, feeParam)
	suite.ctx = suite.ctx.WithMinGasPrices([]sdk.DecCoin{sdk.NewDecCoinFromDec("atom", sdk.NewDec(100)), sdk.NewDecCoinFromDec("unos", sdk.NewDec(1))})
	// 750000 < 15000 * 100
	...
	suite.app.ParamsKeeper.SetFeesParams(suite.ctx, feeParam)
	suite.ctx = suite.ctx.WithMinGasPrices([]sdk.DecCoin{sdk.NewDecCoinFromDec("atom", sdk.NewDec(200)), sdk.NewDecCoinFromDec("unos", sdk.NewDec(1))})
	// 750000 < 15000 * 200
	tx, _ = suite.createTestTxWithGas(msg, 750000, 15000, priv1, "atom")
	_, err = antehandler(suite.ctx, tx, false)
	suite.Assert().ErrorContains(err, "insufficient fees")
	...
	feeParam.GlobalMinimumGasPrices = sdk.NewDecCoins(sdk.NewDecCoinFromDec("unos", sdk.NewDec(50)))
	suite.app.ParamsKeeper.SetFeesParams(suite.ctx, feeParam)
	suite.ctx = suite.ctx.WithMinGasPrices([]sdk.DecCoin{sdk.NewDecCoinFromDec("atom", sdk.NewDec(50)), sdk.NewDecCoinFromDec("unos", sdk.NewDec(1))})
	// 750000 = 15000 * 50
	tx, _ = suite.createTestTxWithGas(msg, 750000, 15000, priv1, "atom")
	_, err = antehandler(suite.ctx, tx, false)
	suite.Require().Nil(err, "Decorator should not have errored on equal fee for global gasPrice")
	...
	feeParam.GlobalMinimumGasPrices = sdk.NewDecCoins(sdk.NewDecCoinFromDec("unos", sdk.NewDec(50)))
	suite.app.ParamsKeeper.SetFeesParams(suite.ctx, feeParam)
	suite.ctx = suite.ctx.WithMinGasPrices([]sdk.DecCoin{sdk.NewDecCoinFromDec("atom", sdk.NewDec(1)), sdk.NewDecCoinFromDec("unos", sdk.NewDec(5))})
	// 750000 = 15000 * 50
	tx, _ = suite.createTestTxWithGas(msg, 750000, 15000, priv1, "unos")
	_, err = antehandler(suite.ctx, tx, false)
	suite.Require().Nil(err, "Decorator should not have errored on equal fee for global gasPrice")
	...
	feeParam.GlobalMinimumGasPrices = sdk.NewDecCoins(
		...
		sdk.NewDecCoinFromDec("unos", sdk.NewDec(2)),
	)
	suite.app.ParamsKeeper.SetFeesParams(suite.ctx, feeParam)
	suite.ctx = suite.ctx.WithMinGasPrices([]sdk.DecCoin{
		sdk.NewDecCoinFromDec("atom", sdk.NewDec(50)),
		sdk.NewDecCoinFromDec("unos", sdk.NewDec(2)), // equal to global minimum
	})
	tx, _ = suite.createTestTxWithGas(msg, 750000, 15000, priv1, "atom")
	_, err = antehandler(suite.ctx, tx, false)
	suite.Require().Nil(err, "Decorator should not have errored, fee is sufficient in 'unos' denomination")
	...
	suite.ctx = suite.ctx.WithMinGasPrices([]sdk.DecCoin{
		...
		sdk.NewDecCoinFromDec("unos", sdk.NewDec(10)),  // greater than global minimum
	})
	...
	// Test case: fee provided in 'unos' denomination is greater than the global minimum gas prices
	feeParam = suite.app.ParamsKeeper.GetFeesParams(suite.ctx)
	feeParam.GlobalMinimumGasPrices = sdk.NewDecCoins(
		sdk.NewDecCoinFromDec("atom", sdk.NewDec(50)),
		sdk.NewDecCoinFromDec("unos", sdk.NewDec(2)),
	...
	tx, _ = suite.createTestTxWithGas(msg, 1500000, 15000, priv1, "unos")
	_, err = antehandler(suite.ctx, tx, false)
	suite.Require().Nil(err, "Decorator should not have errored, fee is sufficient in 'unos' denomination")

	// Test case: fee provided in 'unos' denomination is less than the global minimum gas prices
	feeParam = suite.app.ParamsKeeper.GetFeesParams(suite.ctx)
	feeParam.GlobalMinimumGasPrices = sdk.NewDecCoins(
		sdk.NewDecCoinFromDec("atom", sdk.NewDec(50)),
		sdk.NewDecCoinFromDec("unos", sdk.NewDec(10)),
	...
	suite.ctx = suite.ctx.WithMinGasPrices([]sdk.DecCoin{
		sdk.NewDecCoinFromDec("atom", sdk.NewDec(50)),
		sdk.NewDecCoinFromDec("unos", sdk.NewDec(5)), // less than global minimum
	})
	tx, _ = suite.createTestTxWithGas(msg, 100000, 15000, priv1, "unos")
	_, err = antehandler(suite.ctx, tx, false)
	suite.Assert().ErrorContains(err, "insufficient fees")
}
