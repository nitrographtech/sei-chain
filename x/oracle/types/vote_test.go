...
func TestParseExchangeRateTuples(t *testing.T) {
	valid := "123.0unos,123.123ukrw"
	_, err := ParseExchangeRateTuples(valid)
	require.NoError(t, err)

	duplicatedDenom := "100.0unos,123.123ukrw,121233.123ukrw"
	...
	invalidCoinsWithValid := "123.0unos,123.1"
	_, err = ParseExchangeRateTuples(invalidCoinsWithValid)
	require.Error(t, err)

	abstainCoinsWithValid := "0.0unos,123.1ukrw"
